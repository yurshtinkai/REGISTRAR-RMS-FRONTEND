...
function RequestManagementView({ setDocumentModalData }) {
    ...
    const handleApprove = async (request) => {
        ...
        try {
            ...
            if (response.ok) {
                alert('✅ Request approved successfully! You can now print the document.');
                        ...
                        <div className="table-responsive" style={{ maxHeight: 'calc(100vh - 240px)', overflowY: 'auto' }}>
                            <table className="table table-hover">
                                <thead className="table-dark sticky-top">
                                    <tr>
                                        <th>Student ID</th><th>Student Name</th><th>Doc Type</th><th>Amount</th><th>Status</th>
                                        <th>Notes</th><th>Date</th><th>Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr key={req.id}>
                                        ...
                                        <td><strong>₱{req.amount?.toFixed(2) || '0.00'}</strong></td>
                                        ...
                                        <td>
                                            <div className="d-flex gap-1">
                                                {req.status === 'pending' && (
                                                    <>
                                                        <span className="text-muted small">Awaiting registrar action</span>
                                                    </>
                                                )}
                                                ...
                                                {req.status === 'ready for pick-up' && (
                                                    <>
                                                        ...
                                                        <span className="text-success small ms-2">📋 Ready for pickup</span>
                                                    </>
                                                )}
                                                {req.status === 'rejected' && (
